...
from ivcr.processors import Blip2ImageEvalProcessor
...
class Chat:
    ...
    def answer(self, convs, img_lists, max_new_tokens=300, num_beams=1, min_length=1, top_p=0.9,
        ...
        # print(f"inputs_embeds shape is{embs[0].shape}")
        # print(f"attentiong_mask shape is {attn_mask[0].shape}")
        outputs = self.model.llama_model.generate(
            inputs_embeds=embs.to(torch.float16),
    ...
    def upload_top10_video(self, video_paths, convs, img_lists,n_frms):
        msgs = []
        videos = []
        all_new_msgs = []
        all_timestamps = [] if self.model.qformer_text_input else None
        for top10_video in video_paths:
            v = []
            new_msgs = []
            m = []
            timestamp = []
            for video in top10_video:
                video_content, msg,new_msg = load_video(
                    video_path = video,
                    n_frms = n_frms,
                    height = 224,
                    width = 224,
                ...
                video_content = self.vis_processor.transform(video_content)
                v.append(video_content)
                m.append(msg)
                new_msgs.append(new_msg)
                if self.model.qformer_text_input:
                    timestamps = msg.split('sampled at')[1].replace('seconds.', '').strip().split(
            ...
            msgs.append(m)
            all_new_msgs.append(new_msgs)
            videos.append(v)
        
        video_embeds = []
        for sub_video,sub_timestamp in zip(videos,all_timestamps):
            video_embed = []
            for video,timestamp in zip(sub_video, sub_timestamp):
                video = video.unsqueeze(0).to(self.device)
                times = self.model.tokenizer(
                    timestamp,
                    return_tensors="pt",
                    padding="longest",
                    max_length=32,
                ...
                video_emb, _ = self.model.encode_videoQformer_visual(video, timestamp=times,is_video_clip=False)
                video_embed.append(video_emb.squeeze(0))
            video_embeds.append(video_embed)
        for conv, img_list, msg, video_emb in zip(convs, img_lists, all_new_msgs, video_embeds):
            img_list.extend(video_em.unsqueeze(0) for video_em in video_emb)
            sentence = None
            sentence = "<Video><ImageHere></Video>" + "Please find the video that best matches the query text from the given videos."
            # sentence = "<Video><ImageHere></Video>" + 'There are 10 videos.'
            # for index, m in enumerate(msg):
            #     if index == 0:
            #         sentence = "<Video><ImageHere></Video> " + m
            #     else:
            #         sentence += "<Video><ImageHere></Video> " + m
    ...
    def upload_video_without_audio(self, video_paths, convs, img_lists, n_frms=8):
        ...
        if min(n_frms_in_batch) == max(n_frms_in_batch):  # nb of frames of all videos is ${num_frm}
            ...
            video_embs, _ = self.model.encode_videoQformer_visual(videos, timestamp=all_timestamps,is_video_clip=True)
    ...
    def get_context_emb(self, convs, img_lists, lora=False):
        ...
        for ci, conv in enumerate(convs):
            ...
            img_list = torch.cat([img for img in img_list],dim=1).unsqueeze(0)
