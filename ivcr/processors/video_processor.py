...
import decord
import numpy as np
from ivcr.processors import transforms_video
from ivcr.processors.base_processor import BaseProcessor
from ivcr.processors.randaugment import VideoRandomAugment
from ivcr.processors import functional_video as F
...
def load_video(video_path,n_frms=MAX_INT, height=-1, width=-1, sampling="uniform", return_msg = False,is_video_clip = False):
    decord.bridge.set_bridge("torch")
    ...
    #     img_descri_list = []
    #     # print(video_path)
    #     for frame_count in indices:
    #         image_name = f'frame_{frame_count:04d}'
    #         if image_name not in image_description:
    #             for i in range(5):
    #                 new_image_name = f'frame_{frame_count+i:04d}'
    #                 if new_image_name in image_description:
    ...
    #                     image_name = new_image_name
    #                     break
    #         img_descri = image_description[image_name]
    #         img_descri_list.append(img_descri)
    #     # assert len(indices) == len(img_descri_list)
    #     assert len(indices) == len(img_descri_list), f"AssertionError: Length of indices ({len(indices)}) and img_descri_list ({len(img_descri_list)}) do not match."
    ...
    return frms, msg
    # else:
    #     video_name = video_path.replace('/data/hanning/data/ivcr_compress/','').strip()
    #     msg = f"The video {video_name} contains {len(indices)} frames sampled at {sec} seconds. "
    #     new_msg = f"The video is {video_name}"
    #     return frms, msg,new_msg
